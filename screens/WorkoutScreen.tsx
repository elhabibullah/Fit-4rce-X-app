...
const WorkoutScreen: React.FC = () => {
    ...
    const handleGenerate = async () => {
        ...
        try {
            const prompt = `Generate a ${intensity} intensity ${workoutType} workout. Requirements: ${customRequirements}. Be concise.`;
    ...
    if (view === 'finished') return (
        <div className="flex flex-col items-center justify-center min-h-screen p-6 text-center bg-black">
            <Activity size={56} className="text-green-400 mb-8 animate-pulse" />
            <h2 className="text-lg sm:text-2xl font-black text-white uppercase mb-4 tracking-tight sm:tracking-widest px-2 leading-none">{translate('workout.finished.title')}</h2>
            <p className="text-zinc-500 text-[9px] sm:text-[10px] mb-12 uppercase tracking-widest leading-relaxed max-w-xs mx-auto px-4">{translate('workout.finished.desc')}</p>
            <Button onClick={() => setScreen(Screen.Home)} className="w-full max-w-sm py-6 font-bold text-xs tracking-widest uppercase">{translate('workout.btn.home')}</Button>
        </div>
    );

    return (
        <div className="p-4 sm:p-8 animate-fadeIn max-w-lg mx-auto w-full pt-10 pb-44 overflow-y-auto bg-black custom-scrollbar">
            <header className="text-center mb-12">
                <h1 className="text-lg sm:text-2xl font-black text-white uppercase tracking-tight sm:tracking-widest px-2 leading-none">{translate('workout.setup.title')}</h1>
